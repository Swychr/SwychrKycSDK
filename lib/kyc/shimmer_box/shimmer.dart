...
///
/// * use one [ADShimmer] to wrap list of [Widget]s instead of a list of many [ADShimmer]s
///
@immutable
class ADShimmer extends StatefulWidget {
  ...
  /// [ADShimmer] which [gradient] is [LinearGradient] made up of `baseColor` and
  /// `highlightColor`.
  ///
  ...
  @override
  _ADShimmerState createState() => _ADShimmerState();
...
class _ADShimmerState extends State<ADShimmer> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(ADShimmer oldWidget) {
    if (widget.enabled) {
      _controller.forward();
    } else {
      _controller.stop();
